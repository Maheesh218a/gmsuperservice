...
import { FaPhoneAlt, FaWhatsapp, FaCheckCircle, FaCarSide } from "react-icons/fa";
import { motion } from "framer-motion";
import { doc, getDoc } from "firebase/firestore";
import { db } from "../../firebase/config";

export default function KX2422() {
  const services = [
    "🚘 Wedding & Rent Cars",
  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        ...
        if (docRef.exists()) {
          setImages(docRef.data().kx2422_images || []);
        }
      } catch (error) {
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={containerVariants} className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-32">
            <motion.div variants={itemVariants}>
              ...
              <p className="text-brand-light leading-relaxed text-xl font-light mb-8">
                The KX 2422 is an exquisite luxury car designed for personal, VIP, and business travel. Whether you need elegant transportation for your wedding day, a reliable airport transfer, or a stylish ride for business meetings, this vehicle delivers prestige.
              </p>
              <p className="text-brand-metallic leading-relaxed text-lg">
                Experience unparalleled comfort at every turn.
              </p>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="glassmorphism-dark p-10 md:p-14 rounded-[3rem] border border-brand-accent/20 relative overflow-hidden">
              ...
              <h2 className="text-3xl font-bold text-white mb-10 tracking-tight">Services Available</h2>
